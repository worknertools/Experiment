(() => {
  ...
  const ctx = canvas.getContext('2d', { alpha: false }); // 优化：无 alpha，提升填充性能（无读操作）
  ...
  function detectEdges(imageData, edgeThresholdValue, detailLevelValue) {
    ...
    const edges = [];
    /* ---- 关键改动：线性阈值衰减，调整为1-30范围，detail=1 → 1x (粗轮廓)，detail=30 → 0.2x (细细节)，变化更渐进丰富 ---- */
    const factor = 1 - (detailLevelValue - 1) / 29 * 0.8; // 1~30 → 1~0.2
    const threshold = Math.max(edgeThresholdValue * factor, 0.5);
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        const idx = y * width + x;
        ...
        let d;
        if (dir < 22.5 || dir >= 157.5) d = 0;else
        if (dir < 67.5) d = 45;else
        if (dir < 112.5) d = 90;else
        d = 135;
        let n1 = 0,n2 = 0;
        ...
        if (magnitude[idx] >= n1 && magnitude[idx] >= n2) edges.push(idx);
      }
    }
    return edges;
  }
  ...
  function renderPreview() {
    ...
    for (let i = 0; i < cachedEdges.length; i += step) {
      ...
      /* ---- 整数坐标，避免浮点开销 ---- */
      const drawX = Math.floor(canvasOffsetX + x * uniformScale - thicknessOffset);
      const drawY = Math.floor(canvasOffsetY + y * uniformScale - thicknessOffset);
      const cx = Math.floor(drawX + rectSize / 2);
      const cy = Math.floor(drawY + rectSize / 2);
      const cr = Math.floor(radius); // 半径也整数
      if (styleMode === 'square') {
        ctx.rect(drawX, drawY, rectSize, rectSize); // 添加矩形子路径
      } else {
        ctx.moveTo(cx + cr, cy); // 新子路径起点
        ctx.arc(cx, cy, cr, 0, Math.PI * 2); // 添加圆子路径
      }
    }
    ctx.fill(); // 单次填充所有形状（巨大性能提升）
    updateCursor();
  }
  ...
  edgeSlider.addEventListener('input', event => {
    ...
    cachedEdges = null; // 立即重算
    scheduleRedraw(); // 立即渲染
    updateEdges(); // throttle 防抖
  });
